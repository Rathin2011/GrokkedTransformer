"""
Fixed copy of causal_tracing_composition.py (original untouched):
  - device pinned to cuda:0 instead of the hardcoded cuda:5 (crashes on any
    node without 6 visible GPUs).
  - optional --checkpoint arg to run the full protocol against just ONE
    checkpoint instead of every checkpoint in the directory (avoids paying
    for the full 300-example x 7-layer x 3-perturbation-type protocol on
    every saved checkpoint when we only want the best/final one).
Otherwise byte-identical logic to the original: 300 train_inferred examples,
target_layer=8, MRR probes at layers 1-7, causal patching of h/r1/r2 at
layers 1-7 with manual per-layer forward re-execution.
"""
import numpy as np
import json
import os
...
def main():
    ...
    parser.add_argument("--checkpoint", default=None, type=str, help="run only this one checkpoint (e.g. checkpoint-230000)")
    ...
    with open(args.save_path, "w", encoding='utf-8') as f:
        json.dump(results, f)
    print("DONE, wrote", args.save_path)
